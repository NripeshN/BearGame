function Bear() {
  ...
  class Bee {
    constructor(beeNumber) {
      ...
      //iits HTML ID this.id = this.htmlElement.id;
      //the left position (x)
      this.x = this.htmlElement.offsetLeft;
      //the top position (y)
      this.y = this.htmlElement.offsetTop;
  ...
  function GameOver() {
    ...
    document.getElementById("speedBees").innerHTML = 0;
    alert("Game Over");
    restart_game();
  }
  ...
  function isHit(defender, offender) {
    if (overlap(defender, offender)) {
      ...
      if (score >= 1000) {
        GameOver();
      } //increment the score hits.innerHTML = score; //display the new score //calculate longest duration
